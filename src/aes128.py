...
def aes128_enc(plain, key):
    if not check_args(plain, key):
        raise TypeError("msg or key must be an array of 16 bytes coded as int")
    state = \
        [
...
def aes128_dec(cipher, key):
    if not check_args(cipher, key):
        raise TypeError("msg and key must be an array of 16 bytes coded as int")
    state = \
        [
...
def inv_shift_rows(state):
    s_0 = state[0]
    s_1 = [state[1][3], state[1][0], state[1][1], state[1][2]]
    s_2 = [state[2][2], state[2][3], state[2][0], state[2][1]]
    s_3 = [state[3][1], state[3][2], state[3][3], state[3][0]]
    return [s_0, s_1, s_2, s_3]
...
def check_args(msg, key):
    if (not isinstance(msg, list)) or (not isinstance(key, list)):
        return False
    if len(msg) != 16 or len(key) != 16:
        return False
    for i in range(0, 16):
        if not isinstance(msg[i], int) or (not isinstance(key[i], int)):
            return False
        if msg[i] not in range(0, 256) or key[i] not in range(0, 256):
            return False
    return True
...
def main():
    ...
    print("Encryption:")
    ciphertext = aes128_enc(plaintext, raw_key)
    print("Ciphertext:", ciphertext)

    print("Decryption:")
    plaintext = aes128_dec(ciphertext, raw_key)
